...
def makeMSGFs(con, params):
    ...
    for record in con.execute("SELECT id, basename, dirname FROM files ORDER BY basename DESC"):
        msgfPre = writePrefile(con, record['id'], record['basename'])
        msgf = runMSGF(msgfPre, record['dirname'], enzyme)
...
def importMSGF(con, msgf):
    with open(msgf, 'r') as f:
        ...
        for line in f:
            ...
            if prob == "N/A:":
                prob = 0
            data.append( (prob , line[keys['ScanName']]) )
